        ...
        return <div>
             <Card className='profile-card round-border' >
                ...
                <div className='pAbsolute w-100' style={{display:showEditPage? 'block':'none', zIndex:'10',top:'-30px', background:'rgb(0,0,0,0.3)'}} fluid>
                   ...
                   <p>{profile.title}
                        Strive School
                        {profile.area} <span className='h6 bold blue-link'>Contact info</span></p>
                    <p><span className='h6 bold blue-link'>96 Connections</span></p>
                    <div>
                        <Button onClick={(e)=>showDropdown(e.target.innerText)} className='h6 bold rounded-btn' style={{backgroundColor:'rgb(9, 79, 168)'}}>Open to</Button>
                        <Button onClick={(e)=>showDropdown(e.target.innerText)} variant="outline-secondary" className='h6 bold rounded-btn ml-2'>Add Section</Button>
